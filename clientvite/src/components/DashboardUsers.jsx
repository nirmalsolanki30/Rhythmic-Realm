  ...
  useEffect(() => {
    if (!allUsers) {
      getAllUsers().then((data) => {
        console.log(data);
        console.log('rendering dashboard users')
        dispatch({
    ...
    className='relative w-full rounded-md flex items-center justify-between py-4 bg-lightOverlay cursor-pointer hover:bg-card hover:shadow-md'>
...

      {
        data._id !== user?.user._id && (
          <motion.div whileTap={{scale : 0.75}} className='absolute left-4 w-8 h-8 rounded-md flex items-center justify-center bg-stone-900' onClick={() => deleteUser(data._id)}>
      ...
      {
        data._id !== user?.user._id && (
          <motion.p whileTap={{scale : 0.75}} className='text-[10px] font-semibold text-textColor px-1 bg-teal-400 rounded-sm hover:shadow-md' onClick={()=>setIsUserRoleUpdated(true)}>
...

      {
        ...
        <div className='flex items-center gap-4'>
          <motion.button whileTap={{scale : 0.75}} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-green-500 text-black hover:shadow-md' onClick={()=>updateUserRole(data._id, data.role === "admin" ? "member" : "admin")}>
            Yes
          </motion.button>
          <motion.button whileTap={{scale : 0.75}} className='outline-none border-none text-sm px-4 py-1 rounded-md bg-red-500 text-black hover:shadow-md' onClick={()=>setIsUserRoleUpdated(false)}>
            No
          </motion.button>
        </div>
      </motion.div>
...
const DashboardUsers = () => {
  ...
  return (
    <div className='w-full p-4 flex items-center justify-center flex-col'>
      ...
      {/* tabular data format */}
      <div className='relative w-full py-12 min-h-[400px] overflow-x-scroll overflow-y-scroll my-4 flex flex-col items-center justify-start p-4 border border-gray-300 rounded-md gap-3'>
